import { useEffect } from 'react';
import Navbar from '../components/Navbar';
import Footer from '../components/Footer';
import ContactSection from '../components/ContactSection';
import GoogleAdsHero from '../components/google-ads/Hero';
import GoogleAdsWhyItMatters from '../components/google-ads/WhyItMatters';
import GoogleAdsHowItWorks from '../components/google-ads/HowItWorks';
import GoogleAdsPricing from '../components/google-ads/Pricing';

const GoogleAds = () => {
  // SEO: Set meta tags and structured data
  useEffect(() => {
    document.title = 'Google Ads Management | Lead Generation Campaigns for UK Businesses';
    
    const setMetaTag = (property: string, content: string, isProperty = false) => {
      const attribute = isProperty ? 'property' : 'name';
      let element = document.querySelector(`meta[${attribute}="${property}"]`);
      
      if (!element) {
        element = document.createElement('meta');
        element.setAttribute(attribute, property);
        document.head.appendChild(element);
      }
      
      element.setAttribute('content', content);
    };

    setMetaTag('description', 'Expert Google Ads management for local lead generation. Targeted search campaigns that capture ready-to-buy leads with measurable ROI. 3x-5x return on ad spend typical.');
    setMetaTag('keywords', 'Google Ads management, PPC campaigns, lead generation, local advertising, search ads, conversion optimization, Google Ads agency');
    setMetaTag('robots', 'index, follow');
    setMetaTag('author', 'Van Borg Limited');
    
    setMetaTag('og:type', 'website', true);
    setMetaTag('og:url', 'https://vanborglimited.com/google-ads', true);
    setMetaTag('og:title', 'Google Ads Management | Lead Generation Campaigns', true);
    setMetaTag('og:description', 'Expert Google Ads management for local lead generation. Targeted search campaigns with measurable ROI.', true);
    setMetaTag('og:image', 'https://vanborglimited.com/og-image.jpg', true);
    setMetaTag('og:site_name', 'Van Borg Limited', true);
    
    setMetaTag('twitter:card', 'summary_large_image', true);
    setMetaTag('twitter:url', 'https://vanborglimited.com/google-ads', true);
    setMetaTag('twitter:title', 'Google Ads Management | Lead Generation Campaigns', true);
    setMetaTag('twitter:description', 'Expert Google Ads management for local lead generation. Targeted search campaigns with measurable ROI.', true);
    setMetaTag('twitter:image', 'https://vanborglimited.com/og-image.jpg', true);
    setMetaTag('twitter:site', '@VanBorgLimited', true);
    setMetaTag('twitter:creator', '@VanBorgLimited', true);
    
    let canonicalLink = document.querySelector('link[rel="canonical"]') as HTMLLinkElement;
    if (!canonicalLink) {
      canonicalLink = document.createElement('link');
      canonicalLink.rel = 'canonical';
      document.head.appendChild(canonicalLink);
    }
    canonicalLink.href = 'https://vanborglimited.com/google-ads';
    
    return () => {
      document.title = 'Van Borg Limited';
    };
  }, []);

  return (
    <div className="min-h-screen bg-white">
      <Navbar />
      
      <main className="pt-24 sm:pt-28 lg:pt-32 pb-6 sm:pb-8 lg:pb-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <GoogleAdsHero />
        </div>
      </main>

      {/* Pricing full-width directly under hero */}
      <GoogleAdsPricing />

      {/* Remaining content inside constrained layout */}
      <section className="pb-12 sm:pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <GoogleAdsHowItWorks />
          <GoogleAdsWhyItMatters />
        </div>
      </section>
      
      <ContactSection />
      <Footer />
    </div>
  );
};

export default GoogleAds;